 ...
 * 
 * Typically, one constructs an image with VehicleImage2D(length_m, width_m, vehType, isACC, color,
 * scale),
 * 
 * sets its state variables: setScale(scale) [optional] setPosition(x_m, y_m) setSpeed(speed_ms)
 * [optional] setDirection(phi) [optional]
 * 
 * and draws it: draw(g, colorCode) colorCode=0:physical color; 1:speed colormap; 2:physical color
 * if ACC, gray otherwise
 * 
 * For labelling purposes, one can also set the pixel coordinates directly: setPositionPix(xPix,
 * yPix)
 * 
 * 
 * Units of variables with names *_m in meter, ALL others in pixels!
 * 
 * Physical units: x_m=0 <=> left, y_m=0 <=> bottom,
 * 
 * pixel units: x=0 <=> left corner of drawing region, y=0 <=> top (!) corner of drawing region.
 */
...
public class VehicleImage2D implements Constants
{
    ...
    /**
     * hue values (see, e.g., http://help.adobe.com/en_US/Photoshop/11.0/images/wc_HSB.png):
     * h=0:red, h=0.2: yellow, h=0.35: green, h=0.5: blue, h=0.65: violet, then a long magenta
     * region
     */
